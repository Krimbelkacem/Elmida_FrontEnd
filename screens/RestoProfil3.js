...
export default function Resto({ route, navigation }) {
  ...
  const [timeStatusopen, setTimeStatusopen] = useState("");
  const [timeStatusclose, setTimeStatusclose] = useState("");
  const fetchStatus = async () => {
    try {
      ...
      setTimeStatusopen(response.data.openingTime);

      setTimeStatusclose(response.data.closingTime);
    } catch (error) {
    ...
    <View style={{ backgroundColor: "#FFF" }}>
      ...
      >
            ...
            <View>
              {display ? (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("Paramettres", {
                      idR: Resto._id,
        ...
        <Animated.View entering={FadeInRight.delay(300).duration(300)}>
          ...
          {timeStatusopen ? (
            <Text style={styles.title}>Ouvre à {timeStatusopen}</Text>
          ) : timeStatusclose ? (
            <Text style={styles.title}>ferme à {timeStatusclose}</Text>
          ) : (
            <Text style={styles.title}>fermer pour la journee</Text>
          )}
