...
const Recherche = ({ navigation }) => {
  ...
  const [suphighPriceResto, setSuphighPriceResto] = useState([]);
  ...
  const getDataUsingSimpleGetCall = async () => {
    if (restoName.trim() !== "") {
      try {
        ...
        setSuphighPriceResto(data.suphighPriceResto);
  ...
  const data = [
    ...
    { key: "3", value: "000" },
    { key: "4", value: "0000" },
    { key: "5", value: "null", disabled: true },
  ];
  ...
  const renderRestoList = () => {
    ...
    } else if (selectedValue === "0000") {
      return suphighPriceResto.map((resto) => (
        <Animated.View
          key={resto.id}
          entering={FadeInRight.delay(300).duration(400)}
        >
  ...
  const buttons = [
    `Tous (${totalLength})`,
    `Restaurants (${restoResults?.length})`,
    `Plats (${itemLength})`,
    ...
    if (selectedIndex === 0) {
      ...
      return (
        <View>
          ...
          <View style={{ minHeight: 600 }}>{renderRestoList()}</View>
        </View>
      );
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            >
              <TextInput
                ...
                value={restoName}
                placeholder=" Restaurant , Plat , Une cuisine"
                defaultValue=""
