...
*/
    ...
    try {
      ...
      if (sessionData) {
        ...
        if (user) {
          ...
          getUserSubscribedPhotos(user._id);
        }
      } else {
  ...
  const [subscribedPhotos, setSubscribedPhotos] = useState([]);
  const getUserSubscribedPhotos = async (userId) => {
    try {
      if (userId) {
        // If the user is not logged in
        const response = await axios.post(
          `${API_URL}/getUserSubscribedRestaurantsPhotos?idU=${userId}`
        );
        setSubscribedPhotos(response.data);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <View styles={{ flex: 1, backgroundColor: "white" }}>
      ...
      >
        ...
        {/* <View>{renderSection(homePub.slice(3, 5))}</View>*/}
        ...
        <View>
          ...
          {renderSection(subscribedPhotos)}
        </View>
      </ScrollView>
      {/*isOpen && (
