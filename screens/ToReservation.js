...
const ToReservation = ({ route }) => {
  ...
  const handleSendNotification = (userId) => {
    ...
    const notification = {
      ...
      message: `nouvelle demande de reservation Resto: ${route.params.Resto.name}`,
    };
  ...
  async function handlesubmit() {
    ...
    if (!selectedDay || !selectedTime || !selectedNumber || !restoId) {
      alert("veilliez remplire toutes les selections");
      return;
    }
    ...
    if (sessionData) {
      ...
      if (userId) {
        try {
          ...
          return alert("Demande de reservation envoyer");
        } catch (error) {
          console.error(error);
  ...
  useEffect(() => {
    // Vérification si le restaurant est ouvert lors de la sélection du jour et de l'heure
    Restoopeninghours = route.params.Resto.openingHours;
    console.log("Restoopeninghours", Restoopeninghours);
    if (selectedDay && selectedTime) {
      const selectedDayOfWeek = moment(selectedDay).format("dddd");
      const selectedDateTime = moment(selectedTime, "hh:mm A");

      const openingHour = Restoopeninghours.find(
        (hour) => hour.day === selectedDayOfWeek
      );

      if (openingHour) {
        const startTime = moment(openingHour.startTime, "hh:mm A");
        const endTime = moment(openingHour.endTime, "hh:mm A");

        if (selectedDateTime.isBetween(startTime, endTime)) {
          setRestaurantOpen(true);
        } else {
          setRestaurantOpen(false);
          alert("Le restaurant est fermé à cette heure.");
        }
      } else {
        setRestaurantOpen(false);
        alert("Le restaurant est fermé ce jour-là.");
      }
    } else {
      setRestaurantOpen(false);
    }
